...
class USForm990(Map):
	...
	def install(self, file_name):
		...
		cursor.execute("DROP DATABASE IF EXISTS usform990;")
		cursor.execute("CREATE DATABASE IF NOT EXISTS usform990;")
		cursor.execute("USE usform990;")
		cnx.commit()

		# Need to create a SQL query that creates a table with the right column values
		create_table_query = "CREATE TABLE IF NOT EXISTS manifest (url text, filename varchar(255), doctype varchar(255), year year, ein int(255), name text, formtype int(255), formdate date, size text, assets int(255), ocrstatus text DEFAULT NULL, ocradded datetime DEFAULT NULL, id int(255));"
