...
class USForm990:
	...
	def install(self, file_name):
		...
		cursor.execute("DROP DATABASE IF EXISTS enronemail;")
		cursor.execute("CREATE DATABASE IF NOT EXISTS enronemail;")
		cursor.execute("USE enronemail;")
		cnx.commit()

		# Need to create a SQL query that creates a table with the right column values
		create_table_query = "CREATE TABLE IF NOT EXISTS emails (url text, filename varchar(255), doctype varchar(255), year year, ein int(255), name text, formtype int(255), formdate date, size text, assets int(255), ocrstatus text DEFAULT NULL, ocradded datetime DEFAULT NULL, id int(255));"
		...
		for files in glob.glob("*.csv"):
		    ...
		    #GENERAL INSERT SQL
			add_csv_file = ('''INSERT INTO emails VALUES (%(csv_row)s);''')
